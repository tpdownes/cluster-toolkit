...
| <a name="input_managed_lustre"></a> [managed\_lustre](#input\_managed\_lustre) | Configure Managed Lustre (assumes driver already installed) | <pre>object({<br/>    enabled = optional(bool, false)<br/>    port    = optional(number, 988)<br/>  })</pre> | <pre>{<br/>  "enabled": false,<br/>  "port": null<br/>}</pre> | no |
